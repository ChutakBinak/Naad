...
/**
 * Canvas-based level meter driven by a Web Audio AnalyserNode.
 * Reads time-domain data each frame and paints colour-coded bars:
 *   blue (0–60 %) → amber (60–85 %) → red (85–100 %)
 */
export function LevelMeter({ analyserNode }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // RMS of the time-domain signal, scaled to 0–1
      let sum = 0;
  ...
  return (
    <div className="level-meter-wrap" role="meter" aria-label="Recording level">
      ...
      <canvas ref={canvasRef} className="lm-canvas" width={220} height={20} />
    </div>
  );
}
